...
test.describe('Базовые тесты для getByAltText()', () => {
  ...
  test('Найти изображение по точному alt-тексту', async ({ page }) => {
    ...
    await expect(landscapeImage).toHaveJSProperty('width', 300);
    await expect(landscapeImage).toHaveJSProperty('height', 200);
  });
  ...
  test('Найти логотип компании', async ({ page }) => {
    const logo = page.getByAltText(/логотип/i); // TODO(student): замените на корректный локатор
...
test.describe('Тесты для динамических изображений', () => {
  ...
  test('Найти динамически загруженное изображение', async ({ page }) => {
    ...
    await expect(dynamicImage).toHaveJSProperty('height', 150);
  });
  ...
  test('Найти все иконки по частичному alt-тексту', async ({ page }) => {
    const icons = page.getByAltText(/иконка/i); // TODO(student): замените на корректный локатор
