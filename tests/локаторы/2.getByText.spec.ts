...
test.describe('Поиск по частичному совпадению', () => {
  ...
  test('Найти элемент списка по части текста', async ({ page }) => {
    const listItem = page.locator('li', { hasText: /Специальный/ }); // TODO(student): замените на корректный локатор
...
test.describe('Сложные случаи поиска по тексту', () => {
  ...
  test('Найти вложенный текст', async ({ page }) => {
    const nestedSpan = page.locator('span', { hasText: /вложенным текстом/ }); // TODO(student): замените на корректный локатор
    await expect(nestedSpan).toBeVisible();
    await expect(nestedSpan).toHaveText('вложенным текстом');
    const parent = page.locator('p', { has: nestedSpan });
  ...
  test('Работа с динамическим контентом', async ({ page }) => {
    const dynamicText = page.getByText('Этот текст появился после загрузки страницы'); // TODO(student): замените на корректный локатор
