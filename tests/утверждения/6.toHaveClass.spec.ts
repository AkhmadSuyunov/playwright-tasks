...
test('4. Проверка классов карточки пользователя', async ({ page }) => {
  ...
  await expect(userStatus).not.toHaveClass(/premium/);

  await page.getByRole('button', { name: 'Перейти на Премиум' }).click();
  await expect(userStatus).toHaveClass(/premium/);

  await page.getByRole('button', { name: 'Отметить как просроченный' }).click();
  await expect(userStatus).toHaveClass([/premium/, /expired/]);
  await expect(userStatus).toHaveClass(/premium.*expired|expired.*premium/);
});
