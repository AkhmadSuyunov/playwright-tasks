...
test('1. Проверка атрибутов основной кнопки', async ({ page }) => {
  ...
  await expect(sendBtn).toHaveAttribute('data-action', 'submit');
  await expect(sendBtn).toHaveAttribute('title', 'Основная кнопка');

  await page.getByRole('button', { name: 'Переключить атрибуты' }).click();

  await expect(sendBtn).toHaveAttribute('data-action', 'cancel');
  await expect(sendBtn).toHaveAttribute('title', 'Отмена действия');
});
...
test('3. Проверка атрибутов изображения', async ({ page }) => {
  ...
  const profileImg = page.getByAltText('Аватар пользователя');
  await expect(profileImg).toHaveAttribute('src', 'user.jpg');
  await expect(profileImg).toHaveAttribute('alt', 'Аватар пользователя');
  await expect(profileImg).toHaveAttribute('width', '200');
});
...
test('5. Проверка data-атрибутов', async ({ page }) => {
  ...
  await page.getByRole('button', 'Обновить data-атрибуты').click();
