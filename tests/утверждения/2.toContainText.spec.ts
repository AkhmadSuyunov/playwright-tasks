...
test('4. Проверка скрытого/отображаемого текста', async ({ page }) => {
  ...
  const hiddenText = page.locator('#hidden-content');
  await expect(hiddenText).not.toBeVisible();

  await page.locator('#toggle-text').click();
  await expect(hiddenText).toContainText('special content');
  await expect(hiddenText).toContainText('hidden but now is visible');
});
...
test('5. Проверка частичного совпадения в длинном тексте', async ({ page }) => {
  ...
  const text = page.locator('#partial-text');

  await expect(text).toContainText('quick brown fox');
  await expect(text).toContainText('lazy dog');
  await expect(text).toContainText('all letters of the English alphabet');
  await expect(text).not.toContainText('all letters of the Russian alphabet');
});
